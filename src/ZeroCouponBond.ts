...
export function handleMint(event: MintEvent): void {
  ...
  if (fractionalDeposit == null) {
    ...
    let fractionalDepositContract = FractionalDepositContract.bind(
      event.params.fractionalDepositAddress
    );
    let depositID = fractionalDepositContract.nftID();
    let poolAddress = fractionalDepositContract.pool();
    let depositEntityID =
      poolAddress.toHex() + DELIMITER + depositID.toString();
...
export function handleRedeemFractionalDepositShares(
  event: RedeemFractionalDepositSharesEvent
): void {
  let fractionalDeposit = FractionalDeposit.load(
    event.params.fractionalDepositAddress.toHex()
  )!;
