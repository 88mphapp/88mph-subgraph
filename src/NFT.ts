...
export function handleTransfer(event: ETransfer): void {
  ...
  if (pool) {
    if (stringEqual(type, "deposit")) {
      ...
      if (deposit) {
        if (deposit.active) {
          fromUser.numActiveDeposits = fromUser.numActiveDeposits.minus(
            ONE_INT
          );
        }
        ...
        if (toUser.numActiveDeposits.equals(ZERO_INT) && deposit.active) {
        ...
        let toUserTotalDepositEntity = UserTotalDeposit.load(
          toUserTotalDepositID
        );
      ...
      if (funding) {
        ...
        if (fromFunderTotalInterestEntity) {
